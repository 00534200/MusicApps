<html>
  <head>
    <link rel="stylesheet" href="assets/css/bootstrap.css" />
  </head>
  <body>
		<h2>MusicApps</h2>
    <table>
				<form method="post" action="inscription/traitement">
					<tr>
						<td>nom :</td>
						<td><input style="height:40px" type="text" name="nom"></td>
					</tr>
					<tr>
						<td>prenom :</td> 
						<td><input style="height:40px" type="text" name="prenom"></td>
					</tr>	
					<tr>
						<td>email :</td> 
						<td><input style="height:40px" type="text" name="email"></td>
					</tr>	
					<tr>
						<td>Tapez votre mot de passe :</td> 
						<td><input style="height:40px" type="password" name="mdp"></td>
					</tr>
					<tr>
						<td>Confirmez votre mot de passe :</td> 
						<td><input style="height:40px" type="password" name="mdp1"></td>
					</tr>
					<tr>
						<td><input type="submit" name="Submit" value="Envoyer"></td>
						<td><a href="connecter">Annuler</a></td>
					</tr>	
				</form>
		</table>
  </body>
</html>